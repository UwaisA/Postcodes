...
class ValidatorNoOptionalSeparatorsTest {
    ...
    fun `Constructor with options functions`() {
        Validator(localeBrazil, Options(OptionalSeparator.EXCLUDE))
        Validator(localeBrazil, Options(OptionalSeparator.INCLUDE))
    }

    @Test
    fun `Include optional separators validates positively as expected`() {
        val validatorBrazil = Validator(localeBrazil, Options(OptionalSeparator.INCLUDE))
        assertTrue(validatorBrazil.validate("12345-123"))
        assertTrue(validatorBrazil.partialValidate("12345-1"))
        val validatorUK = Validator(Locale.UK, Options(OptionalSeparator.INCLUDE))
    ...
    fun `Don't include optional separators validates positively as expected`() {
        val validatorBrazil = Validator(localeBrazil, Options(OptionalSeparator.EXCLUDE))
        assertTrue(validatorBrazil.validate("12345123"))
        assertTrue(validatorBrazil.partialValidate("123451"))
        val validatorUK = Validator(Locale.UK, Options(OptionalSeparator.EXCLUDE))
